...
**Область видимости:** глобальная, внутри функций, внутри блоков-
циклов и условных. Переменная видна для всех дочерних конструкций.

**Поднятие (hoisting):**
<br>Все переменные и объявления функций поднимаются вверх своей области видимости.
<br>“Поднимается” только объявление, но не присвоение значения переменным.
<br>Вызов функции может быть как до, так и после объявления ф-ии.
<br>Объявление переменной может быть ниже чем присвоение ей значения.
<br>Все необъявленные переменные становятся глобальными! 
Т.е. присвоив значение `function qqq(){x=3}` и вызвав `qqq()` - 
`x` будет доступна снаружи функции. А если написать `let x = 3`, 
то не будет!

[`'use strict';`](https://ru.stackoverflow.com/questions/435546/%d0%a7%d1%82%d0%be-%d0%b7%d0%bd%d0%b0%d1%87%d0%b8%d1%82-use-strict) - 
включает “строгий режим” и выдаёт ошибки в “неоднозначных” местах.

<br>`var` - переменная будет видна внутри всей функции.
<br>`let` - переменная будет видна только внутри блока **for/if**
<br>`const` - значение не может быть изменено или переназначено. 
Поднятие - как у `let`
...
=== сравнение на идентичность в т.ч. с учётом типа данных.Использовать его!
2 == '2' = true, 2 === '2' = false
...
```
[Приоритет](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Operator_Precedence): 
`||` < `&&` < сравнения < остальные. 
<br>`1 + 1 == 2 && 10 * 10 > 50` >> `true` - даже без ск()бочек

### Типы данных


**Девять типов данных:** boolean, string, symbol, number, 
object, undefined, null, nAn, BigInt 

Оператор `typeof x` / `typeof (x)` - возвращает тип данных аргумента.
<br> `if (typeof(val) === 'object'){...}` - типичная проверка на тип данных
<br> `Boolean ('' || 0 || null || undefined || NaN&& false) === false` >>
`true` `Boolean` - приводит все значения к будевым

`(1 + '2') == 12`; `(''+ 1 + 0) == 10`; `''-1 + 0 = -1;`
`('3' * '8') = 24`; `(4 + 10 + 'px') = '14px'` >> всё это `true`.


### Ключевые слова
`let one = 1, two = 2;` - объявление переменных и присвоение им значений

`var` - устаревшее, `const` - константа, `let` - переменная

`;` - не обязательны! Очень редко реально нужны для разделения строк . 

**Окружение** - оно же - область видимости - то какие переменные 
доступны из определённого места кода. Как правило ограничивается 
{ } - т.е. объектами и функциями. 
<br>Есть тнз. поднятие переменных - и определёные правила о том, что в коде будет исполняться в первую очередь.

функция(аргумент) - вызов функции, возврат результать выполнения.

`console.log(Math.min(2, 4) + 100);` - функция внутри выражения

`let theNumber = Number(prompt("Выбери число", "Текст по умолчанию"));`
<br>`alert("Твоё число – квадратный корень из " + theNumber * theNumber);` -
вызов функии из функции

Фигурные скобки для инструкций – как круглые скобки для выражений.
Последовательность инструкций, заключённая в фигурные скобки, называется блоком.

### Циклы
```jsx

while (exp) { инструкция } // повторять, пока в () true
while (count <= 10) { code } // выйти из цикла, когда в () false
do { инструкция } while (exp); //Делает один проход - до проверки условия.
do { let name = prompt("Who are you?");} while (!name); console.log(name);
for(инициализация; проверка; инкремент) { инструкции }
...
for (let car of cars) {console.log(car)} //по одному выводит car
for...in - проходит только по перечислимым свойствам
for (variable in object) {...} // Не следует использовать для массивов
  variable - очередное имя свойства назначается переменной на каждой итерации.
  object - объект, по чьим свойствам мы проходим

let array = [10, 20, 30, 40, 50]
for (let i in array) { array[i]+=3 }// +3 к каждому члену массива

for (let car in cars) { console.log(car, cars[car] ) } // ключ, значение
// Lamborgini Diabolo // VAZ 2107 // Toyota Camry
```
### Условные операторы
